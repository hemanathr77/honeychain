...
export default function RegisterSeller() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.farm_name.trim()) return setError('Farm name is required.');

    setSubmitting(true);
    try {
      await register({
        name: form.name.trim(), email: form.email, phone: form.phone, password: form.password, role: 'SELLER',
        farm_name: form.farm_name.trim(), village: form.village, district: form.district, state: form.state,
      });
    ...
    <div className="min-h-screen bg-gradient-to-br from-honey-50 via-cream-50 to-forest-50 flex items-center justify-center p-4 py-10">
      <div className="w-full max-w-lg">
        <div className="text-center mb-6">
          ...
          <h1 className="font-display font-black text-2xl text-charcoal-900">Register as a Seller</h1>
          <p className="text-charcoal-500 text-sm mt-1">List your honey products and reach verified customers</p>
          <div className="mt-2 inline-flex items-center gap-1.5 bg-amber-50 border border-amber-200 rounded-full px-3 py-1">
            <span className="text-amber-600 text-xs">⏳</span>
            <span className="text-xs text-amber-700">Your account will be reviewed by an admin before selling</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-card border border-cream-200 p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <p className="text-xs font-semibold text-charcoal-500 uppercase tracking-wide mb-3">Farm Details</p>
              <div className="space-y-3">
                <div>
                  <label className="text-sm font-semibold text-charcoal-700 block mb-1">Farm Name <span className="text-red-500">*</span></label>
                  <input className="input-field" placeholder="e.g. Sundarapandian Honey Farm" type="text" value={form.farm_name} onChange={set('farm_name')} required />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-sm font-semibold text-charcoal-700 block mb-1">Village / Town</label>
                    <input className="input-field" placeholder="Village name" type="text" value={form.village} onChange={set('village')} />
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-charcoal-700 block mb-1">District</label>
                    <input className="input-field" placeholder="District" type="text" value={form.district} onChange={set('district')} />
                  </div>
                </div>
                <div>
                  <label className="text-sm font-semibold text-charcoal-700 block mb-1">State</label>
                  <input className="input-field" placeholder="State" type="text" value={form.state} onChange={set('state')} />
                </div>
              </div>
            </div>
