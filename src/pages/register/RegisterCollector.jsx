...
export default function RegisterCollector() {
  ...
  const [form, setForm] = useState({
    ...
    village: '', district: '', state: '', experience_years: '',
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await register({
        name: form.name.trim(), email: form.email, phone: form.phone, password: form.password, role: 'COLLECTOR',
    ...
    <div className="min-h-screen bg-gradient-to-br from-honey-50 via-cream-50 to-forest-50 flex items-center justify-center p-4 py-10">
      <div className="w-full max-w-lg">
        <div className="text-center mb-6">
          ...
          <h1 className="font-display font-black text-2xl text-charcoal-900">Register as a Collector</h1>
          <p className="text-charcoal-500 text-sm mt-1">Help rescue bee swarms and contribute to colony preservation</p>
          <div className="mt-2 inline-flex items-center gap-1.5 bg-amber-50 border border-amber-200 rounded-full px-3 py-1">
            <span className="text-xs text-amber-700">⏳ Admin verification before accepting rescue requests</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-card border border-cream-200 p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <p className="text-xs font-semibold text-charcoal-500 uppercase tracking-wide mb-3">Location & Experience</p>
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-sm font-semibold text-charcoal-700 block mb-1">Village / Town</label>
                    <input className="input-field" placeholder="Village" type="text" value={form.village} onChange={set('village')} />
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="text-sm font-semibold text-charcoal-700 block mb-1">Experience (years)</label>
                    <input className="input-field" placeholder="e.g. 5" type="number" min="0" value={form.experience_years} onChange={set('experience_years')} />
                  </div>
                </div>
              </div>
            </div>
            ...
            <button type="submit" disabled={submitting} className="btn-primary w-full flex items-center justify-center gap-2 disabled:opacity-60">
              {submitting ? (
                ...
                <>Register as Collector <ChevronRight size={16} /></>
              )}
            </button>
          </form>
